...
public class CreeperAIMixin {
    ...
    // if
    // (creeperEntity.getServer().getPlayerManager().getPlayer(lastTargetPlayer.getUuid())
    // != null) {
    ...
    // }
    //
    // if (lastTargetPlayer.isDead() || lastTargetPlayer.isCreative() ||
    // lastTargetPlayer.isSpectator() || lastTargetPlayer.getWorld() !=
    // lastTargetPlayer.getWorld()) {
    ...
    // .subtract(new Vec3d(downPos.getX() + 0.5, creeperEntity.getY(),
    // downPos.getZ() + 0.5))
    // .normalize();
    //
    // if (Math.abs(targetPlayer.getX() - creeperEntity.getX()) < 4 &&
    // Math.abs(targetPlayer.getY() - creeperEntity.getY()) < 2) {
    // tryJumpForward(creeperEntity, directionToPlayer);
    // } else if (Math.abs(targetPlayer.getX() - creeperEntity.getX()) < 2 &&
    // Math.abs(targetPlayer.getY() - creeperEntity.getY()) < 4) {
    ...
    // }
    //
    // private void tryJumpForward(CreeperEntity creeperEntity, Vec3d
    // playerDirection) {
    // if (!creeperEntity.isOnGround()) return; // Jump only if on the ground
    //
    // //It's working but it only jumps with the same force, i want the force is
    // enough to jump to the player's block
    //
    // Vec3d jumpVelocity = creeperEntity.getRotationVec(0.0F).multiply(0, 0,
    // 0).add((playerDirection.x), 0.55, (playerDirection.z));
    //
