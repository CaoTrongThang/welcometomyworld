...
public class RepairKnowledge extends Item {

    public static final double REPAIR_PERCENT = 10.0; // Repair 5% of durability

    public RepairKnowledge(Settings settings) {
        super(settings);
    }

    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        // Get the RepairKnowledge item being used
        ...
        if (!world.isClient) {
            ...
            // Consume the RepairKnowledge only if something was repaired
            if (repaired) {
                wrenchStack.decrement(1); // Decrease the RepairKnowledge count by 1
