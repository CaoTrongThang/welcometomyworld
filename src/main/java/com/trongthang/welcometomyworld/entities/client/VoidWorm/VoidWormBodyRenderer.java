...
public class VoidWormBodyRenderer extends GeoEntityRenderer<VoidWormPartEntity> {
    ...
    public void render(VoidWormPartEntity entity, float entityYaw, float partialTick, MatrixStack matrices,
        ...
        if (head != null && head.ticksSinceDeath > 0) {
            ...
            float beamIntensity = Math.min(deathProgress > 0.8F ? (deathProgress - 0.8F) / 0.2F : 0.0F, 1.0F);
            Random random = Random.create(432L + entity.getId()); // Offset seed by entity ID so beams differ
            ...
            int beamCount = (int) ((deathProgress + deathProgress * deathProgress) / 2.0F * 40.0F); // Slightly fewer
                                                                                                    // beams for
                                                                                                    // segments
            ...
            for (int i = 0; i < beamCount; ++i) {
                ...
                matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(random.nextFloat() * 360.0F));
                matrices.multiply(
                        RotationAxis.POSITIVE_Z.rotationDegrees(random.nextFloat() * 360.0F + deathProgress * 90.0F));

                Matrix4f matrix4f = matrices.peek().getPositionMatrix();
                float beamLength = random.nextFloat() * 10.0F + 3.0F + beamIntensity * 5.0F; // Smaller beams for body
                float beamWidth = random.nextFloat() * 1.0F + 0.5F + beamIntensity * 1.0F;
                ...
                vertexConsumer.vertex(matrix4f, -0.866F * beamWidth, beamLength, -0.5F * beamWidth).color(r, g, b, 0)
                        .next();
                vertexConsumer.vertex(matrix4f, 0.866F * beamWidth, beamLength, -0.5F * beamWidth).color(r, g, b, 0)
                        .next();
                ...
                vertexConsumer.vertex(matrix4f, 0.866F * beamWidth, beamLength, -0.5F * beamWidth).color(r, g, b, 0)
                        .next();
                vertexConsumer.vertex(matrix4f, 0.0F, beamLength, 1.0F * beamWidth).color(r, g, b, 0).next();
